...
import org.mule.munit.config.NestedProcessorChain;
import org.mule.munit.config.NestedProcessorString;
...
/**
 * OutboundEndpointMessageProcessor invokes the {@link org.mule.munit.MockModule#outboundEndpoint(java.lang.String, java.lang.Object, java.util.List)} method in {@link MockModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object _returnValueType;
    private Object assertions;
    private List<org.mule.api.NestedProcessor> _assertionsType;
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                ...
                if (moduleObject == null) {
                    moduleObject = new MockModuleLifecycleAdapter();
                    muleContext.getRegistry().registerObject(MockModuleLifecycleAdapter.class.getName(), moduleObject);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.munit.adapters.MockModuleLifecycleAdapter"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        MockModuleLifecycleAdapter _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((MockModuleLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            ...
            _castedModuleObject = ((MockModuleLifecycleAdapter) moduleObject);
        }
        try {
            String _transformedAddress = ((String) evaluateAndTransform(_muleMessage, OutboundEndpointMessageProcessor.class.getDeclaredField("_addressType").getGenericType(), null, address));
            Object _transformedReturnValue = ((Object) evaluateAndTransform(_muleMessage, OutboundEndpointMessageProcessor.class.getDeclaredField("_returnValueType").getGenericType(), null, returnValue));
            List<org.mule.api.NestedProcessor> _transformedAssertions = new ArrayList<org.mule.api.NestedProcessor>();
            if ((assertions!= null)&&(assertions instanceof List)) {
                for (MessageProcessor messageProcessor: ((List<MessageProcessor> ) assertions)) {
                    _transformedAssertions.add(new NestedProcessorChain(event, muleContext, messageProcessor));
                }
            } else {
                if ((assertions!= null)&&(assertions instanceof String)) {
                    _transformedAssertions.add(new NestedProcessorString(((String) assertions)));
                }
            }
            retryCount.getAndIncrement();
            _castedModuleObject.outboundEndpoint(_transformedAddress, _transformedReturnValue, _transformedAssertions);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
