...
/**
 * SetMessageProcessor invokes the {@link org.mule.munit.AssertModule#set(java.lang.Object)} method in {@link AssertModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(SetMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("set"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, SetMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, payload));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = castedModuleObject.set(transformedPayload);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("set"), event, e);
        }
    }
