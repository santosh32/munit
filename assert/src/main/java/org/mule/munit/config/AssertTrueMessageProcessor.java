...
/**
 * AssertTrueMessageProcessor invokes the {@link org.mule.munit.AssertModule#assertTrue(java.lang.String, java.lang.Boolean)} method in {@link AssertModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class AssertTrueMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(AssertTrueMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("assertTrue"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedMessage = ((String) evaluateAndTransform(_muleMessage, AssertTrueMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, message));
            Boolean transformedCondition = ((Boolean) evaluateAndTransform(_muleMessage, AssertTrueMessageProcessor.class.getDeclaredField("_conditionType").getGenericType(), null, condition));
            retryCount.getAndIncrement();
            castedModuleObject.assertTrue(transformedMessage, transformedCondition);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("assertTrue"), event, e);
        }
    }
