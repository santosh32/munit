...
/**
 * AssertNullMessageProcessor invokes the {@link org.mule.munit.AssertModule#assertNull(java.lang.String, java.lang.Object)} method in {@link AssertModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class AssertNullMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(AssertNullMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("assertNull"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedMessage = ((String) evaluateAndTransform(_muleMessage, AssertNullMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, message));
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, AssertNullMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, "#[payload]"));
            retryCount.getAndIncrement();
            castedModuleObject.assertNull(transformedMessage, transformedPayload);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("assertNull"), event, e);
        }
    }
