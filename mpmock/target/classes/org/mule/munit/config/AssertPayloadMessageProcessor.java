...
/**
 * AssertPayloadMessageProcessor invokes the {@link org.mule.munit.MPMockModule#assertPayload(java.util.List)} method in {@link MPMockModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class AssertPayloadMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private List<org.mule.api.NestedProcessor> _assertionType;
    private static Logger logger = LoggerFactory.getLogger(AssertPayloadMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(MPMockModuleLifecycleAdapter.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.munit.config.MPMockModuleLifecycleAdapter"), e, this);
            }
        }
        ...
        if (assertion instanceof List) {
            for (MessageProcessor messageProcessor: ((List<MessageProcessor> ) assertion)) {
        ...
        throws MuleException
    {
        ...
        MPMockModuleLifecycleAdapter castedModuleObject = null;
        if (moduleObject instanceof String) {
            castedModuleObject = ((MPMockModuleLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("assertPayload"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MPMockModuleLifecycleAdapter) moduleObject);
        }
        try {
            List<org.mule.api.NestedProcessor> transformedAssertion = new ArrayList<org.mule.api.NestedProcessor>();
            if ((assertion!= null)&&(assertion instanceof List)) {
                for (MessageProcessor messageProcessor: ((List<MessageProcessor> ) assertion)) {
                    transformedAssertion.add(new NestedProcessorChain(event, muleContext, messageProcessor));
                }
            } else {
                if ((assertion!= null)&&(assertion instanceof String)) {
                    transformedAssertion.add(new NestedProcessorString(((String) assertion)));
                }
            }
            retryCount.getAndIncrement();
            castedModuleObject.assertPayload(transformedAssertion);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("assertPayload"), event, e);
        }
    }
