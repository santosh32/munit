...
/**
 * RespondMessageProcessor invokes the {@link org.mule.munit.MPMockModule#respond(java.lang.Object)} method in {@link MPMockModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class RespondMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(RespondMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("respond"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MPMockModuleLifecycleAdapter) moduleObject);
        }
        try {
            Object transformedResponse = ((Object) evaluateAndTransform(_muleMessage, RespondMessageProcessor.class.getDeclaredField("_responseType").getGenericType(), null, response));
            retryCount.getAndIncrement();
            castedModuleObject.respond(transformedResponse);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("respond"), event, e);
        }
    }
