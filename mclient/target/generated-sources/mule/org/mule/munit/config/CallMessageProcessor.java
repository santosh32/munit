...
import org.mule.munit.config.spring.NestedProcessorChain;
import org.mule.munit.config.spring.NestedProcessorString;
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Map<String, Object> _parametersType;
    private Object payload;
    private Object _payloadType;
    private Object responseProcessing;
    private List<org.mule.api.NestedProcessor> _responseProcessingType;
    private static Logger logger = LoggerFactory.getLogger(CallMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(MClientLifecycleAdapter.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.munit.config.MClientLifecycleAdapter"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        MClientLifecycleAdapter castedModuleObject = null;
        if (moduleObject instanceof String) {
            castedModuleObject = ((MClientLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("call"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MClientLifecycleAdapter) moduleObject);
        }
        try {
            String transformedPath = ((String) evaluateAndTransform(_muleMessage, CallMessageProcessor.class.getDeclaredField("_pathType").getGenericType(), null, path));
            Map<String, Object> transformedParameters = ((Map<String, Object> ) evaluateAndTransform(_muleMessage, CallMessageProcessor.class.getDeclaredField("_parametersType").getGenericType(), null, parameters));
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, CallMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, payload));
            List<org.mule.api.NestedProcessor> transformedResponseProcessing = new ArrayList<org.mule.api.NestedProcessor>();
            if ((responseProcessing!= null)&&(responseProcessing instanceof List)) {
                for (MessageProcessor messageProcessor: ((List<MessageProcessor> ) responseProcessing)) {
                    transformedResponseProcessing.add(new NestedProcessorChain(event, muleContext, messageProcessor));
                }
            } else {
                if ((responseProcessing!= null)&&(responseProcessing instanceof String)) {
                    transformedResponseProcessing.add(new NestedProcessorString(((String) responseProcessing)));
                }
            }
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = castedModuleObject.call(transformedPath, transformedParameters, transformedPayload, transformedResponseProcessing);
