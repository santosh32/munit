    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Long _timeoutType;
    private Object responseProcessing;
    private List<org.mule.api.NestedProcessor> _responseProcessingType;
    private static Logger logger = LoggerFactory.getLogger(RequestMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("request"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MClientLifecycleAdapter) moduleObject);
        }
        try {
            String transformedUrl = ((String) evaluateAndTransform(_muleMessage, RequestMessageProcessor.class.getDeclaredField("_urlType").getGenericType(), null, url));
            Long transformedTimeout = ((Long) evaluateAndTransform(_muleMessage, RequestMessageProcessor.class.getDeclaredField("_timeoutType").getGenericType(), null, timeout));
            ...
            resultPayload = castedModuleObject.request(transformedUrl, transformedTimeout, transformedResponseProcessing);
