    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(DispatchMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("dispatch"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MClientLifecycleAdapter) moduleObject);
        }
        try {
            String transformedPath = ((String) evaluateAndTransform(_muleMessage, DispatchMessageProcessor.class.getDeclaredField("_pathType").getGenericType(), null, path));
            Map<String, Object> transformedParameters = ((Map<String, Object> ) evaluateAndTransform(_muleMessage, DispatchMessageProcessor.class.getDeclaredField("_parametersType").getGenericType(), null, parameters));
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, DispatchMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, payload));
            retryCount.getAndIncrement();
            castedModuleObject.dispatch(transformedPath, transformedParameters, transformedPayload);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
