...
public class AddExpectedMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

        private Object value;
        private String _messageType;
        private static Logger logger = LoggerFactory.getLogger(AddExpectedMessageProcessor.class);
        /**
         * Module object
         *
         */
            ...
            throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("addExpected"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedValue = ((String) evaluateAndTransform(_muleMessage, AddExpectedMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, value));
            retryCount.getAndIncrement();
            castedModuleObject.addExpected(transformedValue);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("addExpected"), event, e);
        }
    }
