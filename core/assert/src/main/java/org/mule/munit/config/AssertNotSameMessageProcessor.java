...
/**
 * AssertNotSameMessageProcessor invokes the {@link org.mule.munit.AssertModule#assertNotSame(java.lang.String, java.lang.Object, java.lang.Object)} method in {@link AssertModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class AssertNotSameMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(AssertNotSameMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("assertNotSame"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedMessage = ((String) evaluateAndTransform(_muleMessage, AssertNotSameMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, message));
            Object transformedExpected = ((Object) evaluateAndTransform(_muleMessage, AssertNotSameMessageProcessor.class.getDeclaredField("_expectedType").getGenericType(), null, expected));
            Object transformedValue = ((Object) evaluateAndTransform(_muleMessage, AssertNotSameMessageProcessor.class.getDeclaredField("_valueType").getGenericType(), null, value));
            retryCount.getAndIncrement();
            castedModuleObject.assertNotSame(transformedMessage, transformedExpected, transformedValue);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("assertNotSame"), event, e);
        }
    }
