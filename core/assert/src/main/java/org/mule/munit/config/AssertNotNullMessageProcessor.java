...
/**
 * AssertNotNullMessageProcessor invokes the {@link org.mule.munit.AssertModule#assertNotNull(java.lang.String, java.lang.Object)} method in {@link AssertModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class AssertNotNullMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(AssertNotNullMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("assertNotNull"), event, new RuntimeException("Cannot find the configuration specified by the org.mule.munit.config-ref attribute."));
            }
        } else {
            castedModuleObject = ((AssertModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedMessage = ((String) evaluateAndTransform(_muleMessage, AssertNotNullMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, message));
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, AssertNotNullMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, "#[payload]"));
            retryCount.getAndIncrement();
            castedModuleObject.assertNotNull(transformedMessage, transformedPayload);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("assertNotNull"), event, e);
        }
    }
