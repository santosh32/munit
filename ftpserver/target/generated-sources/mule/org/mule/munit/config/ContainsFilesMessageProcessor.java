    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(ContainsFilesMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(FTPServerModuleLifecycleAdapter.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.munit.config.FTPServerModuleLifecycleAdapter"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        FTPServerModuleLifecycleAdapter castedModuleObject = null;
        if (moduleObject instanceof String) {
            castedModuleObject = ((FTPServerModuleLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("containsFiles"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((FTPServerModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedFile = ((String) evaluateAndTransform(_muleMessage, ContainsFilesMessageProcessor.class.getDeclaredField("_fileType").getGenericType(), null, file));
            String transformedPath = ((String) evaluateAndTransform(_muleMessage, ContainsFilesMessageProcessor.class.getDeclaredField("_pathType").getGenericType(), null, path));
            retryCount.getAndIncrement();
            castedModuleObject.containsFiles(transformedFile, transformedPath);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
