    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _messageProcessorType;
    private Object parameters;
    private List<Object> _parametersType;
    private Object times;
    private Integer _timesType;
    private Object atLeast;
    private Integer _atLeastType;
    private Object atMost;
    private Integer _atMostType;
    private static Logger logger = LoggerFactory.getLogger(VerifyCallMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(MockModuleLifecycleAdapter.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.munit.config.MockModuleLifecycleAdapter"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        MockModuleLifecycleAdapter castedModuleObject = null;
        if (moduleObject instanceof String) {
            castedModuleObject = ((MockModuleLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("verifyCall"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((MockModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedMessageProcessor = ((String) evaluateAndTransform(_muleMessage, VerifyCallMessageProcessor.class.getDeclaredField("_messageProcessorType").getGenericType(), null, messageProcessor));
            List<Object> transformedParameters = ((List<Object> ) evaluateAndTransform(_muleMessage, VerifyCallMessageProcessor.class.getDeclaredField("_parametersType").getGenericType(), null, parameters));
            Integer transformedTimes = ((Integer) evaluateAndTransform(_muleMessage, VerifyCallMessageProcessor.class.getDeclaredField("_timesType").getGenericType(), null, times));
            Integer transformedAtLeast = ((Integer) evaluateAndTransform(_muleMessage, VerifyCallMessageProcessor.class.getDeclaredField("_atLeastType").getGenericType(), null, atLeast));
            Integer transformedAtMost = ((Integer) evaluateAndTransform(_muleMessage, VerifyCallMessageProcessor.class.getDeclaredField("_atMostType").getGenericType(), null, atMost));
            retryCount.getAndIncrement();
            castedModuleObject.verifyCall(transformedMessageProcessor, transformedParameters, transformedTimes, transformedAtLeast, transformedAtMost);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
