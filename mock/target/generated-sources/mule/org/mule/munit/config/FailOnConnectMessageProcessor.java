...
/**
 * FailOnConnectMessageProcessor invokes the {@link org.mule.munit.MockModule#failOnConnect()} method in {@link MockModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class FailOnConnectMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private static Logger logger = LoggerFactory.getLogger(FailOnConnectMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("failOnConnect"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            ...
            castedModuleObject.failOnConnect();
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("failOnConnect"), event, e);
        }
    }
